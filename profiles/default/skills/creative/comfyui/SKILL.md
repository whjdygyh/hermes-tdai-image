...
- `electron-troubleshooting-windows.md` — ComfyUI Desktop app issues: base path after relocation, ToDesktop update, venv/PyTorch CUDA, port binding (Windows/WSL)
- `zimage-turbo-workflow.md` — Headless API mode startup, Z-Image-Turbo T2I workflow nodes, submit/poll/download, proxy bypass in WSL
...
13. **`LoadAudio` → "Invalid audio file: X"** means the file doesn't exist in
    `ComfyUI/input/`. The error comes from `validate_inputs()` in
    `comfy_extras/nodes_audio.py` — it checks `folder_paths.exists_annotated_filepath()`.
    The actual audio loading (PyAV) supports mp3, wav, ogg, opus, flac, m4a, aac.
    **Fix:** copy the audio file to `ComfyUI/input/`, restart or refresh the
    LoadAudio dropdown in the web UI.

14. **LTX-2 audio pipeline** uses dedicated nodes, not the generic LoadAudio:
    - `LTXVEmptyLatentAudio` — create empty audio latent (audio length in frames)
    - `LTXVAudioVAELoader` — load audio VAE model checkpoint
    - `LTXVAudioVAEEncode` — encode external audio into VAE latent
    - `LTXVAudioVAEDecode` — decode VAE latent back to audio
    - `LTXVConcatAVLatent` — concatenate audio + video latents
    - `LTXVSeparateAVLatent` — separate into audio/video streams
    - `LTXVSetAudioRefTokens` — reference audio for lip-sync / speaker identity
    The audio VAE operates at **44100 Hz** internally. Audio shorter than the
    video gets padded; longer audio gets truncated.

16. **LTX-2 audio rejects 48kHz input** — the LTX audio VAE operates at 44100 Hz internally. Feeding a 48kHz mp3/wav causes "Invalid audio file" errors. Always convert to **44.1 kHz** before use. Safe formats: WAV (PCM 16bit/44.1kHz/mono), mp3 (44.1kHz/192kbps/mono), M4A (AAC/44.1kHz/mono). Avoid 48kHz entirely.

17. **Audio file placement for workflows** — the generic `LoadAudio` node only
    sees files in `ComfyUI/input/`. Custom LTX audio paths (e.g. for LipDub
    workflows) may load from other directories. If a node has a `file` widget
    that shows a dropdown, the file must be in `input/`. If it has a raw string
    widget, it can reference absolute or relative paths.
